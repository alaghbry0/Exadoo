...
async def notifications_ws():
    ...
    try:
        # حلقة الاستماع؛ في حالة عدم الحاجة لاستقبال رسائل من العميل يمكن تركها
        while True:
            # في حال أردت استقبال رسائل من العميل يمكنك معالجتها هنا
            _ = await websocket.receive()
            await asyncio.sleep(0.1)
    except Exception as e:
        logging.error(f"خطأ في اتصال WebSocket: {e}")
    finally:
        # عند قطع الاتصال، إزالة الـ websocket من القائمة
...
def broadcast_unread_count(telegram_id, unread_count):
    """
    دالة لإرسال رسالة تحديث (broadcast) للعميل الذي يحمل telegram_id معين
    تحتوي الرسالة على عدد الرسائل غير المقروءة.
    """
