...
async def handle_successful_payment(message: Message):
    if isinstance(message.successful_payment, SuccessfulPayment):  # ✅ التحقق من نوع الرسالة
        try:
            ...
            # 🔹 إرسال طلب إلى API Next.js لتجديد الاشتراك باستخدام `app.aiohttp_session`
            subscribe_url = "https://exadoo.onrender.com/api/subscribe"
            webhook_secret = os.getenv("WEBHOOK_SECRET")

            session = current_app.aiohttp_session
            if not session:
                logging.error("❌ جلسة aiohttp غير متاحة!")
                return await message.answer("⚠️ خطأ داخلي، يرجى المحاولة لاحقًا.")
            ...
            try:
                ...
                logging.error(f"❌ خطأ أثناء الاتصال بـ API الاشتراك: {e}")
